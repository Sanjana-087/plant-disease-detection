...
def unfreeze_resnet50_top_layers(
    ...
    for layer in model.layers:
        if (isinstance(layer, keras.Model) or hasattr(layer, "layers")) and "resnet50" in layer.name.lower():
            base_model = layer
            break
    ...
    if base_model is not None:
        ...
        for layer in base_model.layers[freeze_until:]:
            layer.trainable = True
        logger.info("Unfroze top %d nested ResNet50 layers.", n)
    else:
        ...
        if not backbone_layers:
            raise ValueError("Could not find ResNet50 base layers inside the given model.")
        ...
        for layer in backbone_layers[freeze_until:]:
            layer.trainable = True
        logger.info("Unfroze top %d flattened ResNet50 layers (total backbone=%d).", n, total_layers)
